...
sub analyze_output
{
    ...
    if (  $line =~ /^\s*sat\s*$/ )
    {
	#note that the transformation into smt2 negates the formula
	#into the counterexample formulation.
	#Thus, the conjecture is refuted if the SMT solver returns sat.
	$self->result("false");
    }

    elsif (  $line =~ /^\s*unsat\s*$/ )
    {
	#note that the transformation into smt2 negates the formula
	#into the counterexample formulation.
	#Thus, the conjecture is proven if the SMT solver returns unsat.
	$self->result("true");
    }
}
